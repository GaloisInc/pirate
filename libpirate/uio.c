...
static shmem_buffer_t *uio_buffer_init(int fd) {
  int success;
  ...
  success = 0;
  while (!success) {
    uint64_t init = atomic_load(&uio_buffer->init);
    switch (init) {
    case 0:
      if (atomic_compare_exchange_weak(&uio_buffer->init, &init, 1)) {
        success = 1;
      }
      break;
    case 1:
    ...
    case 2:
      return uio_buffer;
    default:
      munmap(uio_buffer, getpagesize());
      return NULL;
    }
  }

  uio_buffer->size = getpagesize() - sizeof(shmem_buffer_t);

  atomic_store(&uio_buffer->init, 2);
  return uio_buffer;
}
...
int uio_buffer_close(int flags, shmem_buffer_t *uio_buffer) {
  ...
  return munmap(uio_buffer, getpagesize());
}
