...
int pirate_shmem_udp_open(int gd, int flags, pirate_channel_t *channels) {
  ...
  return shmem_udp_buffer_open(gd, flags, pathname, &channels[gd]);
#else
  (void)gd;
  (void)flags;
  (void)channels;
...
int pirate_shmem_udp_close(int gd, pirate_channel_t *channels) {
  ...
  if (channels[gd].shmem_buffer != NULL) {
    rv = shmem_udp_buffer_close(gd, channels[gd].shmem_buffer);
  } else {
    errno = ENODEV;
    return -1;
  }
...
ssize_t pirate_shmem_udp_read(shmem_buffer_t *shmem_buffer, void *buf,
                              size_t count) {
#ifdef PIRATE_SHMEM_FEATURE
  return shmem_udp_buffer_read(shmem_buffer, buf, count);
#else
  (void)shmem_buffer;
  (void)buf;
  (void)count;
...
ssize_t pirate_shmem_udp_write(shmem_buffer_t *shmem_buffer, const void *buf,
                               size_t count) {
#ifdef PIRATE_SHMEM_FEATURE
  return shmem_udp_buffer_write(shmem_buffer, buf, count);
#else
  (void)shmem_buffer;
  (void)buf;
  (void)count;
