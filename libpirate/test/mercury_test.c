...
void *test_mercury_request_thread(__attribute__((unused)) void *unused) {
  return (void *)test_mercury_request();
}

void *test_mercury_echo_thread(__attribute__((unused)) void *unused) {
  return (void *)test_mercury_echo();
}


enum greatest_test_res test_communication_pthread_mercury() {
    int rv = -1;
    ...
    rv = pthread_create(&echo_id, NULL, test_mercury_echo_thread, NULL);
    if (rv != 0) {
        FAILm(strerror(rv));
    }
