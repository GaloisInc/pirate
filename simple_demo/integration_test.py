...
def main():
    high = subprocess.Popen(
        ["valgrind", "--leak-check=full", "--error-exitcode=1", "./high", "8081"])
    low = subprocess.Popen(
        ["valgrind", "--leak-check=full", "--error-exitcode=1", "./low", "8080"])
    expected_high = pathlib.Path("index.html").read_text()
    expected_low = pathlib.Path("index.filtered.html").read_text()
    time.sleep(1)
    req_high = requests.get('http://localhost:8081')
    req_low = requests.get('http://localhost:8080')
    observed_high = req_high.text
    observed_low = req_low.text
    if expected_high != observed_high:
        print("did not receive expected response from http://localhost:8081")
        sys.exit(2)
    if expected_low != observed_low:
        print("did not receive expected response from http://localhost:8080")
        sys.exit(2)
    low.terminate()
    high.terminate()
    low.communicate()
