...
class PirateCppConfigurationProvider implements vscode.DebugConfigurationProvider {
    private type: DebuggerType;
    private provider: IConfigurationAssetProvider;
    ...
    async provideDebugConfigurations(folder?: vscode.WorkspaceFolder, token?: vscode.CancellationToken): Promise<vscode.DebugConfiguration[]> {
        ...
        const yaml: string | undefined = this.runPDL(tmpdir);
        if (!yaml) {
            return configs;
        }

        newConfig.name = "Debug Pirate Application";

        tmpdir.removeCallback();
        return configs;
    }

    private runPDL(tmpdir: tmp.DirResult): string | undefined {
        const procOpts: child_process.SpawnSyncOptions = {
            cwd: tmpdir.name,
            encoding: 'utf8',
            shell: true
        };
        const cmd: string = "pdl pdl.yaml debug -o " + tmpdir.name;
        const pdlResult: child_process.SpawnSyncReturns<Buffer> =
            child_process.spawnSync(cmd, procOpts);
        ...
        vscode.window.showInformationMessage("No errors from PDL");
...
export class PirateCppDbgConfigurationProvider extends PirateCppConfigurationProvider {
    public constructor(provider: IConfigurationAssetProvider) {
        super(provider, DebuggerType.cppdbg);
    }
}
