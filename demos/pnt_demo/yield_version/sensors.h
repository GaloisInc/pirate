...
class GpsSensor
{
 ...
 public:
  GpsSensor(SendChannel<Position> &c, Position const& p, Velocity const& v) : _c(c), _p(p), _v(v) { }
...
class RfSensor
{
  SendChannel<Distance> &_c;
  Distance _d;
  Velocity _v; // only used for simulation

 public:
  RfSensor(SendChannel<Distance> &c, Distance const& d, Velocity const& v) : _c(c), _d(d), _v(v) { }
