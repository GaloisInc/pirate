...
#define PAL_NO_TYPE          ((pal_env_type_t)0)
...
#define PAL_ERR_TOOBIG 3 // Adding to env would exceed PAL_MSG_MAX
 ...
 * would grow it larger than `PAL_MSG_MAX` bytes. If the buffer cannot be
 * (re)allocated, returns a negative `errno` value.
 */
...
static pal_env_iterator_t __attribute__((unused))
pal_env_iterator_start(pal_env_t *env)
{
    return env->buf;
}

static pal_env_iterator_t __attribute__((unused))
pal_env_iterator_end(pal_env_t *env)
{
    return &env->buf[env->size];
}

static pal_env_size_t __attribute__((unused))
pal_env_iterator_size(pal_env_iterator_t it)
{
    return *(pal_env_size_t *)it;
}

static pal_env_iterator_t __attribute__((unused))
pal_env_iterator_data(pal_env_iterator_t it)
{
    return it + sizeof(pal_env_size_t);
}

static pal_env_iterator_t __attribute__((unused))
pal_env_iterator_next(pal_env_iterator_t it)
{
